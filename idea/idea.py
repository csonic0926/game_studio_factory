#!/usr/bin/env python3
"""Idea Factory entry, authority validator, and product-thesis compiler.

The Idea Factory accepts a sparse user seed and/or an early game repository,
then lets one AI producer complete the missing product reasoning.  The AI may
recommend or, when explicitly delegated, decide.  This tool keeps provenance
visible and compiles only adopted/repository/delegated material decisions into
canonical product authority for downstream factories.
...
INPUT_SCHEMA_VERSION = "product_thesis_input.v1"
PROBE_SCHEMA_VERSION = "idea_factory_repo_probe.v1"
CONSTRAINTS_SCHEMA_VERSION = "factory_constraints.v1"
RESULT_SCHEMA_VERSION = "idea_factory_result.v1"

IDEA_FACTORY_DIALOGUE_REQUIRED = "IDEA_FACTORY_DIALOGUE_REQUIRED"
...
def start_idea_factory(game_repo_text: str, *, max_candidates: int = 120) -> IdeaFactoryResult:
    game_repo = _resolve_game_repo(game_repo_text)
    if _canonical_complete(game_repo):
    ...
    return probe_repository(game_repo_text, max_candidates=max_candidates)


def probe_repository(game_repo_text: str, *, max_candidates: int = 120) -> IdeaFactoryResult:
    game_repo = _resolve_game_repo(game_repo_text)
    ...
    payload = {
        ...
        "status": IDEA_FACTORY_DIALOGUE_REQUIRED,
        "repository": {
            **snapshot,
        ...
        "interpretation_warning": (
            ...
            "be exploratory or accidental. The AI producer must distinguish repo commitments, "
            "user direction, recommendations, delegated decisions, and validation hypotheses."
        ),
    }
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.command == "start":
            result = start_idea_factory(args.game_repo, max_candidates=args.max_candidates)
