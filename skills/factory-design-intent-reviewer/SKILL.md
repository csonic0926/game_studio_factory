...
--role intent_experience` for that exact package and the cited original rules.
Review the whole package, all active authority, every decision and every required
finding—not an author-selected subset. Never read the completeness review before
finishing your first pass. Never edit design/production or replace human taste.
Write `factory_review.v2` under the supplied game-owned path using
